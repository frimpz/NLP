...
class SqlGen:
    ...
    def getAttributes(self,df):
        ...
        x = df.query("  (rel == 'dobj' & head == %s) |(rel == 'nsubj')|(rel == 'conj:and' & head ==  %s)" %(1,1)).to_dict()
        self.attributes = (x['lemma'])
        self.rel = (x['rel'])
        if self.attributes:
            return x
        else:
            x = df.query("  (rel == 'dobj') | (rel == 'nsubj')").to_dict()
            self.attributes = (x['lemma'])
            self.rel = (x['rel'])
            return x
    ...
    def getValueNodes(self,index):
        ...
        if pos:
            pos = list(pos.keys())[0]
            #will replace with begin with nmod except nmod:of
...
def run(query):
    if query != "":
        ...
        #print(att)
        for i in att:
            x,y,z,tkey = sm.getSimilarAttribute(i)
        ...
        try:
            #print("Similarity : ", z)
            where,positions = sm.whereSim(list(Sql.conditions.values()),table)
        except NameError:
            pass
        except ValueError:
            pass
        ...
        try:
            if where:
                ...
                positions = ner.NER().ner_pass3(positions)
        except:
            NameError

        #print(query)

        return query,thekey,positions
