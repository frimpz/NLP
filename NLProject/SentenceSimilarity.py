...
def getBestAttribute(table,attribute):
    ...
    sim_measure = 0
    tkey = -1
    for thekey,i in enumerate(db.dbaseSchema[table]):
        ...
        if temp_sim_measure > sim_measure:
            ...
            tkey = thekey
    return best_attribute,sim_measure,tkey
...
def getSimilarAttribute(check):
    ...
    for key in db.dbaseSchema:
        ...
        temp_best_attr, temp_best_attr_sim_measure,thekey = getBestAttribute(key, attribute_from_query)
    ...
    #print("table : ", table_from_query, key,temp_table_sim_measure,"attribute : "
        ...
        if temp_measure >sim_measure:
            ...
            if temp_best_attr:
                ...
                attkey = (table_name,thekey)
    return table_name,atttribute_name,sim_measure,attkey
...
def whereSim(values,tables,rec=0):
    ...
    for i in values:
        sim = 0
        for j in tables:
            for kkk,k in  enumerate(db.dbaseSchema[j]):
                temp = (measure_similarity(rep(i),k) + measure_similarity(rep(i),j))/2
                #print(i,j,k,temp)
                if temp > sim:
                    sim = temp
                    x = (j+"."+k)
